...
public class MenuTexte {
  ...
  public static Equipe[] initialisationEquipes(Plateau p) {  
	...
	System.out.println("Qu'elle mode de jeux choisissez-vous ? 1) Joueur vs Joueur, 2) Joueur vs IA, 3) IA VS IA");
	...
	case 1:
		...
		ans[1] = new Equipe(choisitPays(), p, Constante.EQUIPE_DEUX, new Coordonnees(p.getLargeur() - 1,
                p.getLongueur() - 1));
	...
	case 2:
		...
		ans[0] = new Equipe(choisitPays(), p, Constante.EQUIPE_UN, new Coordonnees(0, 0));
		choisirRobot(nbRobot, Constante.EQUIPE_UN, false);
		ans[1] = new IA("IA", p, Constante.EQUIPE_DEUX, new Coordonnees(p.getLargeur() - 1,
	                p.getLongueur() - 1));
  ...
  private static void choisirRobot(int nbRobot,int noEquipe, boolean bot){
	...
	//List<Robot> neo = new ArrayList<Robot>();
	while (nbRobot != 0) {
		...
		int choix = (!bot) ? Constante.secureInput(1, listeRobotPourLesfeignants.length) : (rd.nextInt(listeRobotPourLesfeignants.length)+1);
		switch (choix) {
			case 1:
