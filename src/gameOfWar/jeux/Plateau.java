...
public class Plateau {
	...
	/**
	 * Construit un plateau longueur*largeur avec un certain pourcentage
	 * d'obstacles
	 * 
	 * @param longueur
	 ...
	 *            le pourcentage d'obstacles que le plateau doit avoir
	 */
	public Plateau(int longueur, int largeur, int pourcentageObstacle) {
		/*
		 * on doit avoir un chemin entre les deux bases : on met des murs
		 * seulements au coordonnees 0+1 et max-1 pour avoir un chemin qui fait
		 * le tour
		 */
		...
		double nbObstacles = ((double) pourcentageObstacle / 100)
				* (this.plateau[0].length * this.plateau.length);
		...
		while (currentObstacles < nbObstacles) {
			...
			if (!getCellule(rdX, rdY).estMur()
					&& getCellule(rdX, rdY).estBase() == 0
					&& getCellule(rdX, rdY).contientMine() == 0
					&& getCellule(rdX, rdY).estRobot() == 0) {
	...
	public void pathFinding(Coordonnees source, Coordonnees fin) {
		...
		while (!searchPath.equals(fin)) {
			path.add(searchPath);
			if (searchPath.getHauteur() != this.y
					&& searchPath.getLargeur() != this.x) {
				i = rd.nextInt(2);
				switch (i) {
				case 0:
				...
				searchPath = searchPath.ajout(new Coordonnees(0, 1));
			System.out.println("(" + searchPath.getLargeur() + ","
	...
	public Cellule getCelluleByCoordonnees(Coordonnees coordonnees) {
		/**
		 * un simple parcourt des cellules on renvoie celle possedant les memes
		 * coordonnees que param
		 */
	...
	public void poserRobot(int x, int y, Robot robot) {
		/*
		 * on peut poser un robot si la cellule passer en param n'est pas un
		 * mur, qu'elle ne contienne pas de mine
		 */
		if (!this.estMur(x, y)) {
	...
	public void retirerRobot(Coordonnees coordonnees) {
		this.plateau[coordonnees.getHauteur()][coordonnees.getLargeur()]
				.retirerRobot();
	}
